...
- [ ] In production, all users can only submit `species_id = 'cat'` via the frontend form
- [ ] Admin override: set `app_metadata: { "is_admin": true }` on the user via Supabase Dashboard — no migration or redeployment needed
- [ ] Enforced in the server action: if `species_id !== 'cat'` and `user.app_metadata.is_admin` is not true, reject with an error (do not silently override the value)
- [ ] Frontend may reflect the restriction (hide non-cat options for non-admins), but the server action is the authoritative check
